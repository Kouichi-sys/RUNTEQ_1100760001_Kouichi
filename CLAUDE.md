...
- media_type (string) — "image"(スクリーンショット)または "video"(クリップ)
- seconds (integer) — クリップの長さ(秒)。スクリーンショットは0
- taken_at (datetime)
- memo (text)
...
- ライブ映像閲覧(カメラ一覧は静止画、カメラ個別の画面は動画。通信帯域に配慮)
- 過去映像検索・再生(巻き戻し・早送り)
- スクリーンショット保存(表示中の1コマを画像で保存)
- クリップ保存(開始・終了を指定して動画を切り取る)
- 保存したものの詳細閲覧・編集・削除(本人のもののみ操作可能な権限制御が必要)
- マイページ(自分の保存済みクリップ一覧)
- カメラ列(サーバー)切り替え
